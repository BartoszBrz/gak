...
<article class="gak-article-modal">
    ...
    <div class="gak-event-header" style="background: url(<?php the_field('obrazek_baner_glowny') ?>) no-repeat top center fixed, #100d08;">
    <div class="gak-category-box">
        ...
        <div class="uk-container gak-modal-padding-left">
            
            <h1 class="gak-event-header__text"><?php the_title(); ?></h1>
            <div class="gak-event-details" uk-grid>
                <div class="gak-event-details__element">
                    <img src="<?php echo get_stylesheet_directory_uri(); ?>/assets/images/full_logo/<?php foreach(get_the_category() as $category) { echo strtolower(str_replace(' ', '', $category->cat_name)); } ?>-full-logo.svg" alt="Logo placówki" title="Logo placówki" />
                </div>
                ...
                <?php
                $terms  = wp_get_post_terms( $post->ID, 'dla_kogo' );
                if ($terms) {
                    ?>
                <div class="gak-event-details__element">
                    <p>dla kogo?</p>
            ...
            <div class="uk-container uk-container-expand-left gak-breadcrumbs-events">
                ...
                <div class="gak-breadcrumb-container">
                    <ul class="gak-breadcrumb">
                        <li><a href="" class="uk-modal-close gak-breadcrumb-back">___wstecz</a></li>
                    </ul>
                </div>
                
                <div class="gak-transport">
                    <?php while ( have_rows('mapa_google') ) {
                        the_row();
                        $howToGetThere = get_sub_field('lokalizacja');
                    ?>
                        <h2 class="gak-transport__text">jak dojechać?</h2>
                        <ul class="gak-transport__ul">
                            <li class="bus">
                                <a title="Komunikacją miejską" id="jak_dojade" href="http://trojmiasto.jakdojade.pl/?fn=SKAD_JEDZIESZ&amp;tn=<?php foreach(get_the_category() as $category) { echo $category->cat_name; }?>&amp;tc=<?php echo $howToGetThere['lat']; ?>:<?php echo $howToGetThere['lng']; ?>&amp;ia=true"
                                   ></a>
                            </li>
                            <li class="car">
                                <a title="Samochodem" id="google" href="https://maps.google.com/maps?directionsmode=driving&amp;dirflg=d&amp;daddr=<?php echo $howToGetThere['lat']; ?>+<?php echo $howToGetThere['lng']; ?>8&amp;hl=pl"
                                ></a>
                            </li>
                            <li class="bike">
                                <a title="Pieszo lub rowerem" id="google2" href="https://maps.google.com/maps?directionsmode=bicycling&amp;dirflg=b&amp;daddr=<?php echo $howToGetThere['lat']; ?>+<?php echo $howToGetThere['lng']; ?>&amp;hl=pl"
                                ></a>
                            </li>
                        </ul>
                    <?php } ?>
            ...
            <div class="gak-event-grid uk-grid-large uk-margin-large-top" uk-grid>
                <div class="uk-width-expand">
                    <div class="gak-event-editable">
                        ...
                        <img class="gak-hidden-sharing" src="<?php the_field('obrazek_baner_glowny') ?>" alt="Główna grafika wpisu" title="Główna grafika wpisu" />
